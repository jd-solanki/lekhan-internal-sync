...
export const useUserStore = defineStore('user', () => {
  ...
  */
  ...
  async function deactivateUser(userId: number, name?: string) {
    ...
    const deactivateRes = await authClient.admin.updateUser({
      userId,
    ...
    if (deactivateRes.error) {
      errorToast({
        ...
        description: deactivateRes.error.message || 'Unable to deactivate this user.',
      })
    }
    else {
      successToast({
  ...
  async function reactivateUser(userId: number, name?: string) {
    ...
    const reactivateRes = await authClient.admin.updateUser({
      userId,
    ...
    if (reactivateRes.error) {
      errorToast({
        ...
        description: reactivateRes.error.message || 'Unable to reactivate this user.',
      })
    }
    else {
      successToast({
  ...
  async function deactivateCurrentAccount() {
    ...
    if (res.error) {
      errorToast({
        title: 'Error',
        description: 'Unable to deactivate your account.',
      })
    }
    else {
