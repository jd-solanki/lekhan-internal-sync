...
// Source: https://github.com/Yizack/nuxt-webhook-validators/blob/main/src/runtime/server/lib/helpers.ts
...
export async function computeSignature(secretKey: string, algorithm: webcrypto.Algorithm, payload: string, options?: Partial<{
  extractable: boolean
...
export async function verifyPublicSignature(publicKey: string, algorithm: webcrypto.Algorithm, payload: string, signature: string, options?: Partial<{
  extractable: boolean
  encoding: BufferEncoding
...
export function ensureConfiguration<T extends keyof RuntimeConfig['webhook']>(provider: T, event?: H3Event) {
  if (configContext.provider)
    provider = configContext.provider as T
  const runtimeConfig = useRuntimeConfig(event).webhook[provider]
  if (configContext.provider)
    configContext.provider = null
...
export async function sha256(payload: string | object, encoding?: BufferEncoding) {
...
export async function validateSha256(hash: string, payload: string, options?: Partial<{
  encoding: BufferEncoding
}>) {
