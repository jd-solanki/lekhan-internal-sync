...
All environment variables are validated using [Zod](https://zod.dev) at startup in `shared/libs/env.ts`. This ensures type-safety and catches missing/invalid config before runtime.

```ts [shared/libs/env.ts]
